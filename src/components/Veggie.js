...
const Veggie = () => {
  ...
  useEffect(() => {
    if (check) {
      ...
      const transformProducts = (data) => {
        ...
        localStorage.setItem("veggie", JSON.stringify(loadedProducts));
        console.log(loadedProducts, "fff");
      };

      getProducts(
        {
          url: `https://api.spoonacular.com/recipes/random?apiKey=d1419b5e27184520a7f75d5f1dd7f674&number=9&tags=vegetarian`,
        },
        transformProducts
      );
    }
  ...
  return (
    <div className="mt-5 mb-5 foods popular">
      <div className="container">
        <h2 className="text-center mb-5">Vegetarian Recipes</h2>
        <Splide
          options={{
            perPage: 4,
            breakpoints: {
